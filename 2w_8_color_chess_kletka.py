...
# считаем разницу по горизонтали и вертикали, сразу по модулю, т.к. вторая
# клетка может быть раньше первой
dif_h = abs(h2 - h1)
dif_v = abs(v2 - v1)
...
if (dif_h % 2) == 0:  # если разница по горизонтали кратна 2
    if (dif_v % 2) == 0:  # если еще и разница по вертикали кратна 2
        print('YES')
    else:
        print('NO')
# 2 - опишем второй случай - клетки в через столбец и т.д.
elif (dif_h + 1) % 2 == 0:  # если разница по горизонтали  + 1 кратна 2
    if (dif_h + 1) % 2 == 0:  # если еще и разница по горизонтали кратна 2
        print('YES')
    else:
        print('NO')
else:
